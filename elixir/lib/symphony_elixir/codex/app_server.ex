defmodule SymphonyElixir.Codex.AppServer do
  ...
  defp remote_launch_command(workspace, issue) when is_binary(workspace) do
    ...
    [
      ...
      "exec #{String.trim("env #{env_prefix}")} #{Config.settings!().codex.command}"
    ]
    |> Enum.join(" && ")
  end

  defp codex_environment(issue) do
    settings = Config.settings!()

    [
      {"TICKET_SYSTEM", settings.tracker.kind},
      {"TICKET_ID", issue_identifier(issue)},
      {"VCS_PROVIDER", settings.vcs.provider},
      {"VCS_REPO", settings.vcs.repo}
    ]
  ...
  defp issue_identifier(%{id: id}) when is_binary(id) and id != "", do: id
